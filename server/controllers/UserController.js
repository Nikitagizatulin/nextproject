...
export default {
  async register(req, res) {
    ...
    try {
      ...
      res.cookie('jwt', jwtSignUser(userData.toJSON()), { httpOnly: true });
      res.json({
        user: userData,
      });
    } catch (e) {
  ...
  async login(req, res) {
    try {
      ...
      if (!user) {
        return res.status(400).json({ email: { message: 'User with such email not found!' } });
      }
  ...
  async forgot(req, res) {
    ...
    smtpTransport.sendMail(mailOptions, (err) => {
      if (err) return res.status(400).json('Having trouble sending email');
